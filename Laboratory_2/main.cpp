...
int main() {
    ...
    yAxis[0].color = sf::Color::Blue; // Цвет оси
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    std::string status;
                    float y1 = abs(mathX) - 5;


                    if (mathY > y1 and mathX > 0) {
                        status = "Zone 1";
                    }
                    else if (mathY < y1 and mathX < 0) {
                        status = "Zone 2";
                    }
                    else if (mathY > y1 and mathX < 0) {
                        status = "Zone 3";
                    ...
                    coordinatesText.setString("Coordinates: (" + std::to_string(mathX) + ", " + std::to_string(mathY) + ")" + " " + status);
                }
            }
        }
        ...
        drawGraph(window, [](float x) { return abs(x) - 5.0f; }, -10, 10, 30, 30, sf::Color::Red);
